...
export default function ObjectiveStep({ data, onUpdate }: ObjectiveStepProps) {
  ...
  // Load current skills from localStorage
  useEffect(() => {
    const loadSkills = () => {
  ...
  const generateAIObjective = async () => {
    ...
    // Also extract skills from the current objective text
    ...
    try {
      ...
      console.log("Request URL:", "http://127.0.0.1:5000/api/regenerate");
      ...
      // Set the single rewritten objective (your API returns one, not multiple)
      setGeneratedObjectives([rewrittenText]);
      setShowGeneratedOptions(true);
    } catch (error) {
      console.error("Failed to generate objectives from API:", error);
  ...
  const totalSkills =
    currentSkills.technical.length + currentSkills.soft.length;
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Write a compelling objective that highlights your career goals and key
          strengths, or let AI generate one for you.
        </p>
      </div>
      ...
      {totalSkills > 0 && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <h3 className="text-sm font-medium text-green-900 mb-2">
            Skills Available for AI Generation ({totalSkills} total)
          </h3>
          <div className="flex flex-wrap gap-1">
            {[
              ...currentSkills.technical.slice(0, 8),
              ...currentSkills.soft.slice(0, 4),
            ].map((skill, index) => (
              <span
                key={index}
      ...
      <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-6">
        <div className="flex items-center mb-4">
          ...
          <h3 className="text-lg font-semibold text-purple-900">
            AI-Powered Objective Generator
          </h3>
        </div>
        ...
        <div className="space-y-4">
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                setExperienceLevel(
                  e.target.value as "entry" | "mid" | "senior" | "executive"
                )
              }
            ...
            {generatedObjectives.map((objective, index) => (
              ...
              >
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <button
                    onClick={() => copyToClipboard(objective, index)}
                    className="px-3 py-2 text-gray-600 hover:text-gray-800 text-sm border border-gray-300 rounded-lg hover:border-gray-400 transition-colors flex items-center space-x-1"
                  >
                    {copiedIndex === index ? (
                      <>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          >
            ...
            {showSuggestions ? "Hide" : "Show"} Keyword Library (
            {expandedKeywordSuggestions.length} keywords)
          </button>
              ...
              <div className="max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-3">
                ...
                {filteredKeywords.length === 0 && (
                  <p className="text-gray-500 text-sm text-center py-4">
                    No keywords found matching "{searchKeyword}"
                  </p>
                )}
              </div>
            </div>
