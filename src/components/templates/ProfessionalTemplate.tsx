  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden print:shadow-none print:rounded-none resume-preview-content">
      <div className="h-full p-8 space-y-5 text-sm print:p-6 print:space-y-4">
        ...
        <div className="text-center border-b-2 border-blue-600 pb-4 print:pb-3">
          <h1 className="text-4xl font-bold text-blue-900 mb-3 print:text-3xl print:mb-2 tracking-wide">
            {data.personalInfo.fullName || "Your Name"}
          </h1>

          {/* Contact Information in a single line */}
          <div className="flex flex-wrap justify-center items-center gap-3 text-blue-700 text-sm print:text-xs print:gap-2">
          ...
          {data.personalInfo.location && (
            <div className="mt-1 text-blue-600 text-sm print:text-xs">
              {data.personalInfo.location}
            </div>
          )}
        </div>
        ...
        {data.objective && (
          <div className="print:break-inside-avoid">
            <h2 className="text-lg font-bold text-blue-900 mb-2 print:text-base print:mb-1 uppercase tracking-wide">
              Objective
            </h2>
            <p className="text-blue-800 leading-relaxed text-justify print:text-xs print:leading-relaxed">
              {data.objective}
            </p>
          </div>
        )}
        ...
        {data.education.length > 0 && (
          <div className="print:break-inside-avoid">
            <h2 className="text-lg font-bold text-blue-900 mb-3 print:text-base print:mb-2 uppercase tracking-wide">
              Education
            </h2>
                ...
                <div key={edu.id} className="print:break-inside-avoid">
                  <div className="flex justify-between items-start mb-1 print:mb-0">
                    <div className="flex-1">
                      <h3 className="font-bold text-blue-900 print:text-sm">
                        {edu.degree}
                      </h3>
                      <p className="text-blue-700 italic print:text-xs">
                        {edu.institution}
                      </p>
                      {edu.gpa && (
                        <p className="text-blue-600 print:text-xs">
                          CGPA: {edu.gpa}
                        </p>
                      )}
                    </div>
                    <div className="text-blue-600 text-right print:text-xs font-medium">
                      {formatDate(edu.graduationDate)}
                    </div>
                  </div>
                  {edu.relevantCourses &&
                    edu.relevantCourses.filter((c) => c.trim()).length > 0 && (
                      <p className="text-blue-700 mt-1 print:text-xs print:mt-0">
              ...
              {data.experience.map((exp) => (
                <div key={exp.id} className="print:break-inside-avoid">
                  ...
                  {exp.responsibilities.filter((r) => r.trim()).length > 0 && (
                    <ul className="list-disc list-inside ml-0 space-y-1 text-blue-700 print:ml-0 print:space-y-0 print:text-xs mt-2 print:mt-1">
              ...
              {data.projects.map((project) => (
                <div key={project.id} className="print:break-inside-avoid">
                  ...
                  {project.technologies.length > 0 && (
                    <p className="text-blue-600 text-xs mb-1 print:mb-0 mt-1">
          ...
          <div className="print:break-inside-avoid">
            ...
            <div className="space-y-2 print:space-y-1">
              {data.skills.technical.length > 0 && (
                <div>
                  <span className="font-bold text-blue-900 print:text-sm">
                    Languages:
                  </span>{" "}
                  <span className="text-blue-700 print:text-xs">
            ...
            <div className="space-y-2 print:space-y-1">
              {data.certifications.map((cert) => (
                ...
                >
                  ...
                  <div className="text-blue-600 text-right print:text-xs">
