...
export default function MinimalTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="bg-white shadow-lg rounded-lg overflow-hidden print:shadow-none print:rounded-none resume-preview-content">
      <div className="h-full p-8 space-y-4 text-sm print:p-6 print:space-y-3">
        {/* Header - Name and Contact */}
        <div className="text-center pb-4 print:pb-3">
          <h1 className="text-3xl font-light text-gray-900 mb-2 print:text-2xl print:mb-1 tracking-wide">
            {data.personalInfo.fullName || "Your Name"}
          </h1>

          {/* Contact Information in a single line */}
          <div className="flex flex-wrap justify-center items-center gap-2 text-gray-600 text-xs print:gap-1">
        ...
        {data.experience.length > 0 && (
          <div className="print:break-inside-avoid">
            <h2 className="text-sm font-medium text-gray-900 mb-2 print:text-xs print:mb-1 uppercase tracking-wide">
              Experience
            </h2>
              ...
              {data.experience.map((exp) => (
                <div key={exp.id} className="print:break-inside-avoid">
                  <div className="flex justify-between items-start mb-1 print:mb-0">
                    ...
                    <div className="text-gray-500 text-right print:text-xs">
                      {formatDate(exp.startDate)} -{" "}
                      {formatDate(exp.endDate, exp.current)}
                    </div>
                  </div>
                  {exp.responsibilities.filter((r) => r.trim()).length > 0 && (
                    <ul className="list-disc list-inside ml-0 space-y-1 text-gray-700 print:ml-0 print:space-y-0 print:text-xs mt-1 print:mt-0">
                ...
                <div key={edu.id} className="print:break-inside-avoid">
                  ...
                  {edu.relevantCourses &&
                    edu.relevantCourses.filter((c) => c.trim()).length > 0 && (
                      <p className="text-gray-600 mt-1 print:text-xs print:mt-0">
              ...
              {data.projects.map((project) => (
                <div key={project.id} className="print:break-inside-avoid">
                  ...
                  {project.technologies.length > 0 && (
                    <p className="text-gray-500 text-xs mb-1 print:mb-0 mt-1">
