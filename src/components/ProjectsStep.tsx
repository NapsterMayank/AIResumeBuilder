...
export default function ProjectsStep({ data, onUpdate }: ProjectsStepProps) {
  ...
  const removeProject = (id: string) => {
    ...
    if (expandedId === id) {
      setExpandedId(data.find((project) => project.id !== id)?.id || null);
    }
  };
  ...
  const addTechnology = (projectId: string, tech: string) => {
    ...
    if (project && !project.technologies.includes(tech)) {
      const newTechnologies = [...project.technologies, tech];
  ...
  const generateAIDescription = async (projectId: string) => {
    ...
    if (!project || !project.name) {
      alert(
        "Please fill in the project name first to generate AI description."
      );
      return;
    }
    ...
    try {
      ...
      // Create context text from current project info
      const contextText = `${
        project.name
      ...
      const requestData = {
        inputType: "project_description",
        text: contextText,
        keywords: [...allSkills, ...project.technologies],
        experienceLevel: "mid",
      };
      ...
      console.error("Failed to generate description from API:", error);
      const errorMessage =
  ...
  const generateAIHighlights = async (projectId: string) => {
    const project = data.find((p) => p.id === projectId);
    if (!project || !project.name) {
      alert("Please fill in the project name first to generate AI highlights.");
      return;
    }
    ...
    try {
      ...
      // Create context text from project info
      const contextText = `Project: ${project.name}. Description: ${
        project.description || "No description provided"
      }. Technologies used: ${
        project.technologies.join(", ") || "No technologies specified"
      }. Current highlights: ${project.highlights
        .filter((h) => h.trim())
        .join("; ")}`;

      // Prepare data for API
      const requestData = {
        inputType: "project_highlights",
        text: contextText,
        keywords: [...allSkills, ...project.technologies],
        experienceLevel: "mid",
      };
      ...
      if (!highlightsText) {
        throw new Error("No highlights received from API");
      }

      // Split the response into individual highlights
      ...
      const finalHighlights =
        highlights.length > 0 ? highlights : [highlightsText.trim()];

      updateProject(projectId, { highlights: finalHighlights });
    } catch (error) {
      console.error("Failed to generate highlights from API:", error);
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error occurred";
      alert(
        `API Error: ${errorMessage}. Please check your backend server and try again.`
  ...
  const enhanceExistingHighlight = async (
    projectId: string,
    highlightIndex: number
  ) => {
    const project = data.find((p) => p.id === projectId);
    if (!project || !project.highlights[highlightIndex]?.trim()) {
      alert(
    ...
    try {
      ...
      if (enhancedText) {
        // Take the first bullet point if multiple are returned
        const firstHighlight = enhancedText
          .split(/•|\n/)
          .map((item: string) => item.trim())
          .filter((item: string) => item.length > 10)[0];

        updateHighlight(
          projectId,
          highlightIndex,
          (firstHighlight || enhancedText).trim()
        );
      }
    } catch (error) {
      console.error("Failed to enhance highlight:", error);
  ...
  const getFilteredTechnologies = (projectId: string) => {
    ...
    if (!searchTerm) return allTechnologyKeywords.slice(0, 50); // Show first 50 by default

    return allTechnologyKeywords.filter((tech) =>
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600">
          Showcase your key projects with AI-generated descriptions and
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button
                      ...
                      disabled={
                        aiGeneratingFor === `${project.id}-description` ||
                        !project.name
                      }
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        ...
                        <span>
                          {showTechSuggestions[project.id] ? "Hide" : "Browse"}{" "}
                          200+ Technologies
                        </span>
                      </button>
                    </div>
                    ...
                    {showTechSuggestions[project.id] && (
                      <div className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                        <div className="mb-3">
                          <input
                            type="text"
                            value={techSearchTerm[project.id] || ""}
                            onChange={(e) =>
                              setTechSearchTerm((prev) => ({
                                ...prev,
                        ...
                        <div className="max-h-64 overflow-y-auto">
                          {Object.entries(technologyKeywords).map(
                            ([category, techs]) => {
                              const filteredTechs = techs.filter(
                                (tech) =>
                                  !techSearchTerm[project.id] ||
                                  tech
                                    .toLowerCase()
                                    .includes(
                                      techSearchTerm[project.id].toLowerCase()
                                    )
                              );
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {filteredTechs.map((tech) => (
                                      <button
                                        ...
                                        disabled={project.technologies.includes(
                                          tech
                                        )}
                          ...
                          {getFilteredTechnologies(project.id).length === 0 && (
                ...
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => generateAIHighlights(project.id)}
                        disabled={
                          aiGeneratingFor === `${project.id}-highlights` ||
                          !project.name
                        }
                        className="px-3 py-1 bg-gradient-to-r from-purple-600 to-blue-600 text-white text-sm rounded-lg hover:from-purple-700 hover:to-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center space-x-1"
                      >
                      ...
                      <div key={index} className="flex items-start space-x-2">
                        ...
                        <div className="flex-1">
                          <textarea
                            ...
                            rows={2}
                            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent resize-none"
                            placeholder="Implemented responsive design that improved mobile user engagement by 35%"
                          />
                          {highlight.trim() && (
                            <button
                              onClick={() =>
                                enhanceExistingHighlight(project.id, index)
                              }
                              disabled={
                                aiGeneratingFor ===
                                `${project.id}-highlight-${index}`
                              }
                  ...
                  <div className="mt-2 text-xs text-gray-500">
                    💡 <strong>Pro Tips:</strong> Use quantifiable achievements
                    (e.g., "Improved performance by 40%", "Reduced load time by
                    2 seconds"), and let AI help make your descriptions more
                    professional!
                  </div>
                </div>
              </div>
