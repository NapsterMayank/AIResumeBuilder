...
const MockInterview = () => {
  ...
  const questionTypes = [
    { value: 'technical', label: 'Technical', description: 'Problem-solving and technical skills' },
    { value: 'behavioral', label: 'HR', description: 'Behavioral and HR questions' }
  ];
  const skillTags = [
  ...
  const toggleSkill = (tag: string) => {
    const set = new Set(skills.split(',').map(s => s.trim()).filter(Boolean));
    if (set.has(tag)) {
      set.delete(tag);
    } else {
      set.add(tag);
  ...
  const mockQuestions: Question[] = [
    ...
    },
    {
      ...
      type: 'situational',
      company: 'google',
      question: "If you discovered a critical bug in production just before a major product launch, what would you do?",
      tips: [
  ...
  const filteredQuestions = questions.length ? questions : mockQuestions.filter(q => q.company === selectedCompany && q.type === selectedType);
  ...
  const startInterview = async () => {
    ...
    try {
      ...
      const res = await fetch(API_ENDPOINTS.REGENERATE, {
        ...
        body: JSON.stringify({ inputType: 'mock_interview', text: context, keywords: skills.split(',').map(s=>s.trim()).filter(Boolean), experienceLevel: 'mid' })
      });
      if (res.ok) {
        const data = await res.json();
        const text = data.rewrittenText || '';
        const list = text.split(/\n|•/).map((t:string)=>t.trim()).filter((t:string)=>t.length>10).slice(0,15);
        if (list.length) {
          const diffs: Array<'easy'|'medium'|'hard'> = ['easy','medium','hard'];
          setQuestions(list.map((q, i) => ({ id: String(i+1), type: (selectedType as 'behavioral'|'technical'|'situational'), company: selectedCompany, question: q, tips: ['Use the STAR framework', 'Be concise', 'Mention impact'], difficulty: diffs[i%3] })));
        }
      }
    } catch { /* ignore AI failure and fallback to mock */ }
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-l font-bold text-foreground">AI Mock Interview</h1>
            </div>
            {isInterviewStarted && (
              <div className="flex items-center gap-4">
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <div className="text-center mb-8">
              ...
              <p className="text-xl text-muted-foreground">
                Prepare for your dream job with AI-powered mock interviews from top tech companies
              </p>
            </div>
              ...
              <Card className="bg-gradient-card border-0 shadow-card">
                ...
                <CardContent>
                  <Select value={selectedCompany} onValueChange={setSelectedCompany}>
                    ...
                    <SelectContent>
                      <SelectItem value={"any"}>Any Company (Optional)</SelectItem>
                  ...
                  <div className="grid grid-cols-3 gap-2 mt-4">
                    <Badge 
                      key={'any'}
                      variant={selectedCompany === 'any' ? "default" : "secondary"}
            ...
            <Card className="bg-gradient-card border-0 shadow-card">
              <CardHeader>
                ...
                <CardDescription>Tell us your target role and key skills to tailor questions.</CardDescription>
              </CardHeader>
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Input value={skills} onChange={(e) => setSkills(e.target.value)} placeholder="e.g., React, Node.js, AWS" />
                </div>
                <div className="md:col-span-2">
                  <div className="text-xs text-muted-foreground mb-2">Quick add skills</div>
            ...
            <Card className="bg-gradient-card border-0 shadow-card">
              <CardHeader>
                ...
                <CardDescription>
                  You'll practice {filteredQuestions.length} questions from {companies.find(c => c.value === selectedCompany)?.label}
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {filteredQuestions.slice(0, 3).map((question, index) => (
                    <div key={question.id} className="flex items-center gap-3 p-3 bg-background rounded-lg">
            ...
            <Card className="bg-gradient-primary text-white border-0">
              <CardContent className="p-6 text-center">
                ...
                <p className="text-blue-100 mb-4">
                  Begin your practice session with {filteredQuestions.length} carefully selected questions
                </p>
                <Button 
                  variant="hero" 
                  size="lg"
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="text-center">
                    <div className="text-3xl font-bold text-primary mb-2">{getOverallScore()}%</div>
                  ...
                  <div className="text-center">
                    <div className="text-3xl font-bold text-primary mb-2">
                      {Math.round(answers.reduce((sum, a) => sum + a.timeSpent, 0) / answers.length / 60)}m
                    </div>
              ...
              <CardContent>
                <div className="space-y-4">
                  {answers.map((answer, index) => {
                    ...
                    return (
                      <div key={answer.questionId} className="p-4 border border-border rounded-lg">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex items-center gap-1">
                              {[...Array(5)].map((_, i) => (
                                <Star 
                                  key={i} 
                                  className={`h-4 w-4 ${i < (answer.score || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'}`} 
                                />
                              ))}
                            </div>
                          </div>
                        ...
                        <div className="bg-background p-3 rounded text-sm text-muted-foreground mb-2">
                          <strong>Your answer:</strong> {answer.answer.substring(0, 150)}...
                        </div>
                        <div className="flex items-start gap-2 text-sm">
                          <Lightbulb className="h-4 w-4 text-yellow-500 mt-0.5 flex-shrink-0" />
                          <span className="text-muted-foreground">{answer.feedback}</span>
                        </div>
                      </div>
                    );
                  })}
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge className={`${getDifficultyColor(currentQuestion?.difficulty || 'medium')} text-white`}>
